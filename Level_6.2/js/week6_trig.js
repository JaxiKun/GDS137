...
function animate()
{
	...
	let dx = player.x - pearl.x;
	let dy = player.y - pearl.y;
	let distance = Math.sqrt(dx * dx + dy * dy);

	if (distance < 300) {
	    // Normalize direction
	    let speed = 3; // Adjust speed as needed
	    let angleToPlayer = Math.atan2(dy, dx);
	    pearl.x += Math.cos(angleToPlayer) * speed;
	    pearl.y += Math.sin(angleToPlayer) * speed;
	}

	//--------If the pearl hits the player's x and y coordinates move it off screen.-----------------
	if (Math.abs(pearl.x - player.x) < pearl.width && Math.abs(pearl.y - player.y) < pearl.width) {
	    pearl.x = -1000;
	    pearl.y = -1000;
	}
